...
export function UserProvider({ children }: userProviderProps) {
  ...
  const [currentGroupChat, setCurrentGroupChat] = useState<IGroupChat | null>(
    null
  );
  ...
  useEffect((): (() => void) => {
    socket.current = io(
      process.env.REACT_APP_API_URL || "https://chat-app-api-urzc.onrender.com"
    );
  ...
  useEffect(() => {
    if (newGroupChat) {
      ...
      if (
        newGroupChat.members.filter((member) => member._id === user?._id)
          .length === 0
      ) {
  ...
  useEffect(() => {
    if (currentChat) {
      axios
        .get(
          `${process.env.REACT_APP_API_URL}/groupChat/members/${currentChat}`
        )
        .then((res) => {
  ...
  useEffect(() => {
    if (user) {
      axios
        .get(`${process.env.REACT_APP_API_URL}/groupChat/${user?._id}`)
        .then((res) => {
  ...
  function deleteCurrentChat() {
    axios
      .delete(`${process.env.REACT_APP_API_URL}/chat/${currentChat}`, {})
      .then((res) => {
        setCurrentChat(null);
      })
      .catch((e) => console.log(e));
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/users/login`)
      .then((res) => {
